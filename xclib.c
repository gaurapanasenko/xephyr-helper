      ...
      unsigned long *len, unsigned int *context)
{
	...
	case XCLIB_XCOUT_SENTCONVSEL:
		...
		XGetWindowProperty(dpy,
				   ...
				   0,
				   False,
				   AnyPropertyType, type, &pty_format,
	...
	case XCLIB_XCOUT_INCR:
		...
		XGetWindowProperty(dpy,
				   ...
				   False,
				   AnyPropertyType,
				   type, &pty_format, &pty_items, &pty_size,
				   (unsigned char **)&buffer);
     ...
     unsigned long *pos, unsigned int *context)
{
	...
	case XCLIB_XCIN_NONE:
		...
		*pos = 0;
		...
		if (evt.xselectionrequest.target == targets) {
			...
			/* send INCR response */
			XChangeProperty(dpy, *win, *pty, inc, 32,
					PropModeReplace, 0, 0);
	...
	case XCLIB_XCIN_INCR:
		...
		if (chunk_len) {
			...
			XChangeProperty(dpy,
					*win, *pty, target, 8, PropModeReplace,
					&txt[*pos], (int)chunk_len);
		} else {
			/* make an empty property to show we've
			 * finished the transfer
			 */
			XChangeProperty(dpy, *win, *pty, target, 8,
					PropModeReplace, 0, 0);
		}
		XFlush(dpy);
